...
Процедура ОткрытьОтчетРасходы(Команда)
	...
	Форма = ПолучитьФорму("Обработка.обРасходы.Форма.Форма");
	Форма.Открыть();
...
Процедура ПолучитьДанныеДоходаРасхода()
	...
	Запрос.УстановитьПараметр("КонецПериода",  КонецДня(КонецМесяца(ТекущаяДатаПриОткрытии)));	
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ РасходДенегОбороты.СтатьяРасходов В (&СтатьиРасходовИсключения)";
	
	Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(ТекущаяДатаПриОткрытии));
	Запрос.УстановитьПараметр("КонецПериода",  КонецДня(КонецМесяца(ТекущаяДатаПриОткрытии)));
	СтатьиРасходовИсключения = Новый Массив;
	//СтатьиРасходовИсключения.Добавить(Справочники.СтатьиРасходов.Инвестиции);
	Запрос.УстановитьПараметр("СтатьиРасходовИсключения",  СтатьиРасходовИсключения);
	...
	Запрос.Текст = 
		...
		|	РегистрНакопления.РасходДенег.Обороты(&НачалоПериода, &КонецПериода, , ) КАК РасходДенегОбороты
		|ГДЕ
		|	РасходДенегОбороты.СтатьяРасходов = &СтатьяРасходов";
	
	Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(ТекущаяДатаПриОткрытии));
	Запрос.УстановитьПараметр("КонецПериода",  КонецДня(КонецМесяца(ТекущаяДатаПриОткрытии)));
	Запрос.УстановитьПараметр("СтатьяРасходов",  Справочники.СтатьиРасходов.Инвестиции);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда 
		СуммаПереводаНаИнвестиции = ВыборкаДетальныеЗаписи.СуммаРасходаОборот;
	Иначе
		СуммаПереводаНаИнвестиции = 0;	
	КонецЕсли;
...
&НаКлиенте
Процедура ПредставлениеДатыНажатие(Элемент, СтандартнаяОбработка)
